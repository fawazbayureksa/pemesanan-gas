...
class CrudDistribusi
{
    ...
    public function read()
    {
        ...
        $query = $this->db->query('SELECT * FROM tdistribusi ORDER BY tgl_kirim DESC');
        return $query->fetchAll(PDO::FETCH_ASSOC);
    }
    public function readJoin()
    {
        // Query untuk membaca semua data
        $query = $this->db->query('SELECT * FROM tdistribusi JOIN tmobil ON tdistribusi.id_mobil = tmobil.id JOIN tdriver ON tdistribusi.id_supir = tdriver.id JOIN tpermintaan ON tdistribusi.id_permintaan = tpermintaan.id ORDER BY tgl_kirim DESC');
        return $query->fetchAll(PDO::FETCH_ASSOC);
    }
